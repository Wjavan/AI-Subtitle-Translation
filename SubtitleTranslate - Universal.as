...
string GPT_pre_api_key = "";
string GPT_pre_selected_model = "qwen-flash";
string GPT_pre_apiUrl = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
string GPT_pre_delay_ms = "0";
string GPT_pre_retry_mode = "0";
...
// Unique prefix to avoid collisions with other subtitle plugins.
...
string GPT_selected_model = GPT_pre_selected_model;
string GPT_apiUrl = GPT_pre_apiUrl;
string GPT_delay_ms = GPT_pre_delay_ms;
string GPT_retry_mode = GPT_pre_retry_mode;
string GPT_context_subtitle_count = GPT_pre_context_subtitle_count;
string GPT_context_cache_mode = GPT_pre_context_cache_mode;
string GPT_prompt_cache_retention = GPT_pre_prompt_cache_retention;
string GPT_gemini_cached_content = GPT_pre_gemini_cached_content;
...
array<string> GPT_subtitleHistory;
array<string> GPT_translationHistory; // for context-dedup
...
string ServerLogin(string User, string Pass) {
    ...
    // Bailian: retry without enable_thinking if it was rejected
...
string Translate(string Text, string &in SrcLang, string &in DstLang) {
    ...
    // Skip pure numbers/punctuation/symbols (0 tokens)
    if (!IsTranslatable(Text))
        return Text;

    // Skip onomatopoeia / repetitive text (ASR mishearing background music)
    if (IsOnomatopoeiaOrRepetitive(Text))
        return "";

    // Replace newlines with spaces (avoids delimiter ambiguity)
    ...
    // Retry loop: network errors, API errors, hallucinations
    ...
    while (attempts <= maxRetries) {
        ...
        if (choices.isArray() && choices.size() > 0 &&
            ...
            if (enableOpenAIPromptCacheControls && loweredError.find("prompt_cache") != -1) {
             ...
             // Retry on API errors (rate limits, context length, etc.)
            attempts++;
            continue;
        } else {
...
// Drop echoed >>> delimiter and any translated context lines;
// the translation is what follows the last >>> on the last line.
